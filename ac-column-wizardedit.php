...
class AC_Column_wizardedit extends \AC\Column {

	public function __construct() {

		// Identifier, pick an unique name. Single word, no spaces. Underscores allowed.
		$this->set_type( 'column-wizardedit' );

		// Default column label.
		$this->set_label( __( 'Route Edit WZ', 'ac-wizardedit' ) );
	}
	...
	public function get_raw_value( $post_id ) {
		...
		ob_start();
		?>
		<wm-wizard-container data-conf='{"conf":"{\"feature_id\":\"<?php echo $post_id ?>\"}"}'>
        </wm-wizard-container>
		<?php
		echo ob_get_clean();
	}
	...
	public function scripts() {
		...
		// wp_enqueue_script( 'ac-wizardedit', plugin_dir_url( __FILE__ ) . "js/column.js" );
		// wp_enqueue_style( 'ac-wizardedit', plugin_dir_url( __FILE__ ) . "css/column.css" );
	}
